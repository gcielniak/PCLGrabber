...
#ifdef HAVE_OPENNI2
		//////////////////////////////////////////////////////////////////////////////
		io::Image::Ptr read(const std::string &filename, vector<unsigned char>& buffer)
		{
			...
			return ToImageRGB24(&buffer[0], getWidth(), getHeight());
		}
#endif

#ifdef HAVE_OPENNI
		//////////////////////////////////////////////////////////////////////////////
		openni_wrapper::Image::Ptr readOni(const std::string &filename, vector<unsigned char>& buffer)
		{
			uint32_t uncompressed_size;
			std::vector<char> compressed_data;
			if (!loadImageBlob(filename, compressed_data, uncompressed_size))
			...
			return ToImageRGB24Oni(&buffer[0], getWidth(), getHeight());
		}
#endif
	};
...
#ifdef HAVE_OPENNI2
		//////////////////////////////////////////////////////////////////////////////
		io::DepthImage::Ptr read(const std::string &filename, vector<unsigned short>& buffer, float focal_point)
		{
			uint32_t uncompressed_size;
			std::vector<char> compressed_data;
			if (!loadImageBlob(filename, compressed_data, uncompressed_size))
			...
			return ToDepthImage(&buffer[0], getWidth(), getHeight(), focal_point);
		}
#endif

#ifdef HAVE_OPENNI
		//////////////////////////////////////////////////////////////////////////////
		openni_wrapper::DepthImage::Ptr readOni(const std::string &filename, vector<unsigned short>& buffer, float focal_point)
		{
			uint32_t uncompressed_size;
			std::vector<char> compressed_data;
			if (!loadImageBlob(filename, compressed_data, uncompressed_size))
			...
			return ToDepthImageOni(&buffer[0], getWidth(), getHeight(), focal_point);
		}
#endif
	};
}